...
{       
    ...
    { static string validcheck2 = "false"; 
        static int c = 0;
           ...
           //     try
             //   {
                ...
                if (valid.Value.ToString() == "True")
                {
                    if (bit == 1)
                    {
                        SqlCommand undergradCheck = new SqlCommand("checkUnderGrad", conn);
                        undergradCheck.CommandType = CommandType.StoredProcedure;
                        undergradCheck.Parameters.Add(new SqlParameter("@undergradID", undergradID));
                        SqlParameter validunder = undergradCheck.Parameters.Add("@valid", SqlDbType.Bit);
                        validunder.Direction = System.Data.ParameterDirection.Output;

                        undergradCheck.ExecuteNonQuery();
                        validcheck2 = validunder.Value.ToString();
                    }
                    //check undergrad is unqique



                    if (bit == 0 || validcheck2 == "True")
                    {
